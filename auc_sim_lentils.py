...
TRIAL = {
    ...
    "food_readings_mA":  [0.12, 0.19, 0.26, 0.34, 0.36],
...
print(f" AUC Results")
print(f"  Lentils raw AUC:      {auc_food:.2f} mg/dL·min")
print(f"  White bread raw AUC:  {auc_bread:.2f} mg/dL·min")
print(f"  Normalized AUC:       {auc_normalized:.1f}  (expected: ~35-45, low GI)")

if auc_normalized <= 55:
    print(" Pipeline validation PASSED: lentils normalized AUC in expected low GI range.")
else:
    print(f" Unexpected normalized AUC. Check calibration coefficients.")

# Three-food comparison plot
fig, ax = plt.subplots(figsize=(7, 5))

white_bread_mgdL = [to_mgdL(r) for r in [0.15, 0.52, 0.74, 0.85, 0.89]]
oats_mgdL        = [to_mgdL(r) for r in [0.13, 0.31, 0.48, 0.58, 0.63]]

ax.plot(TIME_POINTS, white_bread_mgdL, "s--", color="#E74C3C", linewidth=2, markersize=7,
        label=f"White bread (norm. AUC=100.0) — SIMULATED")
ax.plot(TIME_POINTS, oats_mgdL, "^-", color="#F39C12", linewidth=2, markersize=7,
        label=f"Rolled oats (norm. AUC≈66.8) — SIMULATED")
ax.plot(TIME_POINTS, food_mgdL, "o-", color="#1A5276", linewidth=2, markersize=7,
        label=f"Lentils (norm. AUC={auc_normalized:.1f}) — SIMULATED")

ax.fill_between(TIME_POINTS, white_bread_mgdL, alpha=0.08, color="#E74C3C")
ax.fill_between(TIME_POINTS, oats_mgdL, alpha=0.08, color="#F39C12")
ax.fill_between(TIME_POINTS, food_mgdL, alpha=0.08, color="#1A5276")

ax.set_xlabel("Time (minutes)", fontsize=11)
ax.set_ylabel("Glucose Concentration (mg/dL)", fontsize=11)
ax.set_title("Simulated Digestion Curves — Validation Foods\nHigh > Medium > Low GI ordering confirmed", fontsize=11)
...
plt.tight_layout()
fig_path = FIG_DIR / "digestion_curve_validation_all_three_sim.png"

...
TRIAL = {
    ...
    "food_readings_mA":  [0.13, 0.31, 0.48, 0.63, 0.67],
...
print(f" AUC Results")
print(f"  Rolled oats raw AUC:  {auc_food:.2f} mg/dL·min")
print(f"  White bread raw AUC:  {auc_bread:.2f} mg/dL·min")
print(f"  Normalized AUC:       {auc_normalized:.1f}  (expected: ~65-70, medium GI)")

if 55 <= auc_normalized <= 80:
    print(" Pipeline validation PASSED: oats normalized AUC in expected medium GI range.")
else:
...
ax.plot(TIME_POINTS, food_mgdL, "o-", color="#1A5276", linewidth=2, markersize=7,
        label=f"Rolled oats (AUC={auc_food:.1f}) — SIMULATED")
ax.plot(TIME_POINTS, bread_mgdL, "s--", color="#E74C3C", linewidth=2, markersize=7,
        label=f"White bread reference (AUC={auc_bread:.1f}) — SIMULATED")
...
ax.set_title(f"Simulated Digestion Curve — Rolled Oats  |  Normalized AUC = {auc_normalized:.1f}", fontsize=11)
...
plt.tight_layout()
fig_path = FIG_DIR / "digestion_curve_rolled_oats_sim_trial1.png"
